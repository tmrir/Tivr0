    ...
    <div className="grid grid-cols-2 gap-4 mb-4 relative">
      <div>
        <label className="flex justify-between items-center text-xs font-bold text-slate-500 mb-1">
          <span>{label} ({t('admin.form.title_ar')})</span>
          <AutoTranslateButton
            text={safeValue.ar}
            onTranslate={translated => onChange({ ...safeValue, en: translated })}
          />
        </label>
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <div className="flex justify-between items-center px-1">
                        <span className="text-[10px] text-slate-400 font-bold uppercase">Ar</span>
                        <AutoTranslateButton text={f.ar} onTranslate={v => updateFeature(i, 'en', v)} />
                      </div>
                      <input className="w-full border p-2 rounded text-sm outline-none" placeholder="Ar" value={f.ar} onChange={e => updateFeature(i, 'ar', e.target.value)} required />
                    </div>
                    <div className="space-y-1">
                      <div className="px-1"><span className="text-[10px] text-slate-400 font-bold uppercase">En</span></div>
                      <input className="w-full border p-2 rounded text-sm outline-none" placeholder="En" value={f.en} onChange={e => updateFeature(i, 'en', e.target.value)} required />
                    </div>
                  </div>
                  <button type="button" onClick={() => removeFeature(i)} className="text-red-500 mt-6"><Trash2 size={18} /></button>
                </div>
              ))}
            </div>
          </div>
                ...
                <div key={i} className="flex gap-3 items-start">
                  <div className="flex-1 grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input className="w-full border p-2 rounded text-sm" value={f.ar} onChange={e => updateFeature(i, 'ar', e.target.value)} required />
                    </div>
                    <div className="space-y-1">
                      <div className="px-1"><span className="text-[10px] text-slate-400 font-bold uppercase">En</span></div>
                      <input className="w-full border p-2 rounded text-sm" value={f.en} onChange={e => updateFeature(i, 'en', e.target.value)} required />
                    </div>
                  </div>
                  <button type="button" onClick={() => removeFeature(i)} className="text-red-500 mt-6"><Trash2 size={18} /></button>
                </div>
        ...
        <SortableList
          ...
          renderItem={(p) => (
            <div className={`relative group transition-all duration-300 ${p.isPopular ? 'scale-105' : ''}`}>
              ...
              {/* Package Card */}
              <div className={`bg-white rounded-2xl p-8 h-full relative overflow-hidden transition-all duration-300 hover:-translate-y-2 ${p.isPopular
                ...
                <div className="text-center mb-6 relative z-10">
                  <div className={`inline-block px-4 py-2 rounded-full text-sm font-bold mb-4 ${p.isPopular
                    ? 'bg-tivro-primary text-white'
                    : 'bg-slate-100 text-slate-600'
                    }`}>
                    {p.id === 'new' ? (lang === 'ar' ? 'باقة جديدة' : 'New Package') : (lang === 'ar' ? 'باقة مميزة' : 'Featured Package')}
                  </div>
                  <h3 className={`text-2xl font-bold mb-3 ${p.isPopular ? 'text-tivro-primary' : 'text-slate-800'
                    }`}>
                    {p.name[lang]}
                  </h3>
                ...
                <div className="space-y-3 mb-6">
                  {p.features.slice(0, 3).map((feature, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${p.isPopular ? 'bg-tivro-primary' : 'bg-slate-300'
                        }`}></div>
                ...
                <div className="flex gap-2 justify-center relative z-20">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-bold transition-all duration-200 flex items-center gap-2 relative z-20 ${p.isPopular
            ...
            {(editing.stats || []).map((stat, i) => (
              <div key={i} className="flex gap-2 items-start mb-2">
                <div className="w-24 mt-5">
                  <input className="w-full border p-2 rounded text-sm font-bold" placeholder="Value" value={stat.value} onChange={e => updateStat(i, 'value', e.target.value)} />
                </div>
                <div className="flex-1 space-y-1">
                  <div className="flex justify-between items-center px-1">
                    <span className="text-[10px] text-slate-400 font-bold">Label (Ar)</span>
                    <AutoTranslateButton text={stat.label.ar} onTranslate={v => updateStat(i, 'labelEn', v)} />
                  </div>
                  <input className="w-full border p-2 rounded text-sm" placeholder="Label (Ar)" value={stat.label.ar} onChange={e => updateStat(i, 'labelAr', e.target.value)} />
                </div>
                <div className="flex-1 space-y-1">
                  <div className="px-1"><span className="text-[10px] text-slate-400 font-bold">Label (En)</span></div>
                  <input className="w-full border p-2 rounded text-sm" placeholder="Label (En)" value={stat.label.en} onChange={e => updateStat(i, 'labelEn', e.target.value)} />
                </div>
                <button type="button" onClick={() => removeStat(i)} className="text-red-500 p-2 bg-white rounded border hover:bg-red-50 mt-5"><Trash2 size={16} /></button>
              </div>
            ))}
      ...
      {editing ? (
        <form onSubmit={handleSave} className="bg-white p-8 rounded-xl shadow-lg border border-slate-200 max-w-3xl mx-auto animate-fade-in">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="flex justify-between items-center text-xs font-bold text-slate-500 mb-1">
                <span>{t('admin.form.content_ar')}</span>
                <AutoTranslateButton
                  text={editing.content.ar}
                  onTranslate={v => setEditing({ ...editing, content: { ...editing.content, en: v } })}
                />
              </label>
...
const PackageRequestsManager: React.FC<ManagerProps> = ({ onUpdate }) => {
  ...
  const loadRequests = async () => {
    try {
      const data = await db.packageRequests.getAll();
      setRequests(data);
    } catch (error) {
      console.error('Failed to load package requests:', error);
    } finally {
      setLoading(false);
    }
  };

  const updateRequestStatus = async (id: string, status: PackageRequest['status']) => {
    try {
      await db.packageRequests.updateStatus(id, status);
      await loadRequests();
      onUpdate();
    } catch (error) {
  ...
  const deleteRequest = async (id: string) => {
    if (confirm(lang === 'ar' ? 'هل أنت متأكد من حذف هذا الطلب؟' : 'Are you sure you want to delete this request?')) {
      try {
  ...
  const getStatusColor = (status: PackageRequest['status']) => {
    switch (status) {
  ...
  const getStatusText = (status: PackageRequest['status']) => {
    const statusTexts = {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <Loader2 className="animate-spin text-tivro-primary" size={32} />
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          {(['all', 'pending', 'contacted', 'completed', 'cancelled'] as const).map(status => (
            <button
              key={status}
            ...
            >
              {status === 'all'
                ? (lang === 'ar' ? 'الكل' : 'All')
                : getStatusText(status as PackageRequest['status'])
              }
            </button>
          ))}
        </div>
      ...
      {filteredRequests.length === 0 ? (
        <div className="text-center py-12 bg-slate-50 rounded-xl">
          <p className="text-slate-500">
            {lang === 'ar' ? 'لا توجد طلبات حالياً' : 'No requests at the moment'}
          </p>
        </div>
      ) : (
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold text-slate-900">{request.customerName}</h3>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(request.status)}`}>
                      {getStatusText(request.status)}
                    </span>
                  </div>
                  <div className="space-y-1 text-sm text-slate-600">
                    <p><span className="font-medium">{lang === 'ar' ? 'الهاتف:' : 'Phone:'}</span> {request.phoneNumber}</p>
                    <p><span className="font-medium">{lang === 'ar' ? 'البريد:' : 'Email:'}</span> {request.email}</p>
                    <p><span className="font-medium">{lang === 'ar' ? 'الباقة:' : 'Package:'}</span> {request.packageName}</p>
                    <p><span className="font-medium">{lang === 'ar' ? 'التاريخ:' : 'Date:'}</span> {new Date(request.createdAt).toLocaleDateString(lang === 'ar' ? 'ar-SA' : 'en-US')}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <select
                    value={request.status}
                    onChange={(e) => updateRequestStatus(request.id, e.target.value as PackageRequest['status'])}
...
const DashboardOverview: React.FC<{ setActiveTab: (tab: string) => void }> = ({ setActiveTab }) => {
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: <Star size={24} />,
      color: 'bg-indigo-500',
      label: lang === 'ar' ? 'طلبات الباقات' : 'Package Requests',
      count: stats.packageRequests,
      onClick: () => setActiveTab('packageRequests')
    }
  ];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {statCards.map((card) => (
          <div
            key={card.key}
            className={`bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow ${card.onClick ? 'cursor-pointer' : ''}`}
